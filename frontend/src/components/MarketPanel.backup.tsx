...
const MarketPanel: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (!binanceStreamsEnabled) {
      console.log('[MarketPanel] Binance streams disabled, chart updates will come from backend WebSocket');
      return;
    }
    
    // If client already exists, just change symbol/interval instead of creating new one
    if (binanceWSClientRef.current) {
      console.log(`[MarketPanel] Updating existing Binance WebSocket to ${selectedSymbol}@kline_${selectedInterval}`);
      
      // Clear chart data when changing interval to prevent time conflicts
      console.log(`[MarketPanel] Clearing chart data for interval change`);
      // Note: We could call clearChart() here if available, but setHistoricalData([]) works too
      setHistoricalData([]);
      
      binanceWSClientRef.current.changeSymbol(selectedSymbol, selectedInterval);
      return;
    }
    
    // Create new Binance WebSocket client for kline data
    console.log(`[MarketPanel] Creating new Binance WebSocket for ${selectedSymbol}@kline_${selectedInterval}`);
    ...
    binanceClient.addListener((data: BinanceKlineData) => {
      ...
      try {
        ...
        const timeValue = Math.floor(data.k.t / 1000); // Ensure it's a number
        const candlestick: CandlestickData = {
          time: timeValue as any, // Convert milliseconds to seconds as number
        ...
        if (isNaN(timeValue) || isNaN(candlestick.open) || isNaN(candlestick.high) || 
            isNaN(candlestick.low) || isNaN(candlestick.close)) {
          console.warn('[MarketPanel] Invalid candlestick data:', candlestick);
          return;
        }
        
        console.log(`[MarketPanel] Updating chart with valid candlestick:`, {
          time: timeValue,
          close: candlestick.close
        });
        updateCandlestick(candlestick);
      } catch (error) {
        console.error('[MarketPanel] Error processing kline data:', error);
      }
  ...
  useEffect(() => {
      ...
      wsClient.addListener((msg) => {
        ...
        switch (msg.type) {
          ...
          case 'orderbook':
            ...
            break;
          // Removed 'kline' case - we use Binance WebSocket for chart data instead
        }
      });
    };
    ...
    return () => {
      ...
      // Also cleanup Binance WebSocket on component unmount
      if (binanceWSClientRef.current) {
        console.log('[MarketPanel] Destroying Binance WebSocket on component unmount');
        binanceWSClientRef.current.destroy();
        binanceWSClientRef.current = null;
      }
    };
